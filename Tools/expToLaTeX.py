...
def exp2LaTeX(num, sig_fig=2):
    ...
    if 'e' in str(num):
        ...
        if len(num2[:num2.find('.')]) > sig_fig:
            ...
            length = len(num2[:num2.find('.')])
            if num2[num2.find('.')+1] >= '5':
                new_num = num2[:sig_fig-1] + str(int(num2[(sig_fig-1)])+1)
            else:
                new_num = num2[:sig_fig]
        else:
            if (sig_fig - len(num2[:num2.find('.')])+1 <= len(num2) and 
                num2[sig_fig - len(num2[:num2.find('.')])+1] >= '5'):
                new_num = num2[:sig_fig] + str(int(num2[sig_fig])+1)
            elif (sig_fig - len(num2[:num2.find('.')])+1 <= len(num2) and 
                num2[sig_fig - len(num2[:num2.find('.')])+1] < '5'):
                new_num = num2[:sig_fig+1] + '0'*(sig_fig-len(num2[:num2.find('.')])-1)
            #el:
            else:
                new_num = num2 + '0'*(sig_fig - len(num2)+1)
        new_num_LaTeX = new_num
    return [new_num, new_num_LaTeX]

if __name__ == '__main__':
    tests = [534321,.002,12.2,2.11,0]
    for i in tests:
        print(exp2LaTeX(i,10))
